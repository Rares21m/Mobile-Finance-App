...
async function getAccounts(req, res) {
  try {
    ...
    if (isBtUnauthorized(err)) {
      logger.warn(
        `BT consent expired for connection ${req.params.connectionId}. Marking as expired.`,
      );
      await prisma.bankConnection
        .update({
...
/**
 * DELETE /api/bt/connections/:bankName
 *
 * Marks all active connections for the given bank (BT or BRD) as "disconnected".
 * The client removes the accounts/transactions from local state.
 */
